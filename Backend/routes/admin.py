...
def all_projects(uid):
    print(get_total_amount('Salim Usman Shaikh','Gpkp Fan repair'))
    conn,cur=db_connection()
...
def add_admin_user():
    ...
    if request.method == 'POST':
        ...
        if existing_user:
            cur.close()
            conn.close()
            return render_template('admin/add_user_form.html', error="Username already exists")
        ...
        try:
            ...
            cur.close()
            conn.close()
            return render_template('admin/add_user_form.html', error="Error adding user")
